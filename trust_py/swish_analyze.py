...
def log_file(log_line, LOG_FILENAME='imdb.log'):
    '''
    log
    '''
    ...
    handler = logging.handlers.RotatingFileHandler(os.path.join(logs_dir, LOG_FILENAME))
    my_logger.addHandler(handler)
    my_logger.info(str(pd.datetime.now())+' '+log_line)
    my_logger.handlers.clear()
    
class tf_model:
    def __init__(self,ACTIVATION_FUNC, UNITS, LAYERS, EPOCHS, BATCH_SIZE, LEARNING_RATE, DROPOUT_RATE,logging_vals=False):
        '''
        ...
        self.logging_vals = logging_vals
    ...
    def build_model(self,X_train, X_test, y_train, y_test):
        ...
        '''
        if self.logging_vals: #log the parameters 
            logs_dir = os.path.join(os.getcwd(), 'logs')
            if not os.path.isdir(logs_dir):
                os.makedirs(logs_dir)
            log_file('Activation Function is '+str(self.ACTIVATION_FUNC)+', Units '+str(self.UNITS)+', Layers '+str(self.LAYERS)+', Epochs '+str(self.EPOCHS)+', Batch Size '+str(self.BATCH_SIZE)+', Learning Rate '+str(self.LEARNING_RATE)+', Dropout Rate '+str(self.DROPOUT_RATE))
        model = tf.keras.Sequential()
        model.add(tf.keras.layers.Dropout(rate=self.DROPOUT_RATE, input_shape=X_train.shape[1:]))
        for _ in range(self.LAYERS-1):
            model.add(tf.keras.layers.Dense(units=self.UNITS, activation=self.ACTIVATION_FUNC))
            model.add(tf.keras.layers.Dropout(rate=self.DROPOUT_RATE))
        model.add(tf.keras.layers.Dense(units=1, activation='sigmoid'))
        optimizer = tf.keras.optimizers.Adam(lr=self.LEARNING_RATE)
        model.compile(optimizer=optimizer,loss='binary_crossentropy', metrics=['accuracy'])
        callbacks = [tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=2)]
        history = model.fit(X_train, y_train, epochs=self.EPOCHS, validation_data=(X_test, y_test), verbose=1, batch_size=self.BATCH_SIZE, callbacks=callbacks)
...
if __name__ == '__main__':
    ...
    for ACTIVATION_FUNC in activation_functions:
        for i in range(iterations):
            activation_frame[ACTIVATION_FUNC].iloc[i] = tf_model(ACTIVATION_FUNC, UNITS, LAYERS, EPOCHS, BATCH_SIZE, LEARNING_RATE, DROPOUT_RATE).build_model(X_train, X_test, y_train, y_test)
    ...
    for ACTIVATION_FUNC in activation_functions:
        ...
        for BATCH_SIZE in batch_list:
            accuracy_score = []
            for i in range(iterations):
                accuracy_score.append(tf_model(ACTIVATION_FUNC, UNITS, LAYERS, EPOCHS, BATCH_SIZE, LEARNING_RATE, DROPOUT_RATE).build_model(X_train, X_test, y_train, y_test))
